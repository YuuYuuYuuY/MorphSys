...
from dgl.nn.pytorch import SGConv, SumPooling, AvgPooling, MaxPooling
from models.extractor.ib import InterBranch


class SgcInterBranch(nn.Module):
    def __init__(self, x_size, h_size, num_classes, num_layers=2, fc=True, bn=False, pool="sumpool", path="add", direction="out", fuse="weighted"):
        super(SgcInterBranch, self).__init__()
        ...
        if bn:
            for i in range(num_layers):
                conv = SGConv(in_feats=h_size, out_feats=h_size, k=2)
