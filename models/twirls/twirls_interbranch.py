...
from dgl.nn.pytorch import TWIRLSConv, SumPooling, AvgPooling, MaxPooling
from models.extractor.ib import InterBranch

class TwirlsInterBranch(nn.Module):
    def __init__(self, x_size, h_size, num_classes, num_layers=2, fc=True, bn=False, pool="sumpool", path="add", direction="out", fuse="weighted"):
        super(TwirlsInterBranch, self).__init__()
        ...
        if bn:
            for i in range(num_layers):
                conv = TWIRLSConv(input_d=h_size, output_d=h_size, hidden_d=h_size, prop_step=64)
                bn = nn.BatchNorm1d(h_size)
                self.convs.append(conv)
                self.batchnorm.append(bn)
        else:
            for i in range(num_layers):
                conv = TWIRLSConv(in_feats=h_size, out_feats=h_size, k=2)
                self.convs.append(conv)
