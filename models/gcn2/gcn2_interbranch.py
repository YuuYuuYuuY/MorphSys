...
from dgl.nn.pytorch import GCN2Conv, SumPooling, AvgPooling, MaxPooling
from models.extractor.ib import InterBranch



class Gcn2InterBranch(nn.Module):
    def __init__(self, x_size, h_size, num_classes, num_layers=2, fc=True, bn=False, pool="sumpool", path="add", direction="out", fuse="weighted"):
        super(Gcn2InterBranch, self).__init__()
        ...
        if bn:
            for i in range(num_layers):
                conv = GCN2Conv(in_feats=h_size, layer=i+1, allow_zero_in_degree=True)
