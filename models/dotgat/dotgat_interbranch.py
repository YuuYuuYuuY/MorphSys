...
from dgl.nn.pytorch import DotGatConv, SumPooling, AvgPooling, MaxPooling
from models.extractor.ib import InterBranch


class DotgatInterBranch(nn.Module):
    def __init__(self, x_size, h_size, num_classes, num_layers=2, fc=True, bn=False, pool="sumpool", path="add", direction="out", fuse="weighted"):
        super(DotgatInterBranch, self).__init__()
        ...
        if bn:
            for i in range(num_layers):
                conv = DotGatConv(self.h_size, self.h_size, 12)
